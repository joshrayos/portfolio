...
const Menubar: FC<Props> = ({ onNavigate }) => {
  ...
  const handleConfirm = useCallback(
    (index?: number) => {
      const targetIdx = index !== undefined ? index : selectedIdx;
      const selection = PAGES[targetIdx].id;
      onNavigate(selection);
    },
    [selectedIdx, onNavigate],
  );
            ...
            <li key={page.id}>
              <button
                className={`flex-1 p-2 lg:w-md cursor-pointer ${
                  index === selectedIdx ? "text-gb-light bg-gb-deep" : ""
                }`}
                onMouseEnter={() => setSelectedIdx(index)}
                onClick={() => handleConfirm(index)}
              >
                {index === selectedIdx ? ">" : ""} {page.label}
              </button>
            </li>
          ))}
        </ul>
